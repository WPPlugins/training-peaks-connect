<?php 
/*
Plugin Name: TrainingPeaks Connect
...
if ( ! class_exists('TrainingPeaks_Connect_Plugin')){
class TrainingPeaks_Connect_Plugin{
	...
	public function TrainingPeaks_Connect_Plugin()
	{
     ...
     function admin_menu()
	{
           add_options_page('TrainingPeaks Connect Options', 'TrainingPeaks Connect',
                             'administrator', __FILE__, 
                              array(&$this, 'plugin_options'));
           add_management_page('TrainingPeaks Connect Accounts', 'TrainingPeaks Connect', 
                                'administrator', __FILE__,
  ...
  if (get_bloginfo('version') >= '2.7') {
            ...
            print <<<EOT
            <div class="wrap">
            <h2>TrainingPeaks Connect</h2>
            <p>This is the Setup page for the TrainingPeaks Connect plug-in.<br>
            This plug-in is recommended for use on WordPress version 2.7 or higher.<p>
               <ul>
               Create a new User Account by going to <a href="$manage_page?page=training-peaks-connect/training-peaks-connect.php">Manage -> TrainingPeaks Connect Accounts</a>, where you will also find more information.<p>
               <li>Display a user's TrainingPeaks data by creating a new User Account and placing the following tag along with the chosen Account Name on any page or post.</li>
               <li><b>-trainingpeaks-connect#AccountName-</b></li>
               <li>-----------------------or-------------------------</li>
               <li><b>-training-peaks-connect#AccountName-</b></li><li><br></li>
               Note: This plug-in does not currently differentiate between private and public workouts, so both will appear on your blog where a shortcode is used.  However the TrainingPeaks.com profile page will still take into consideration which workouts are public/private.<li><br></li>
               
               <br>Data for workouts is retrieved using the TrainingPeaks API.<br><br>
               <a href="http://home.trainingpeaks.com/">TrainingPeaks Homepage</a>
               <br><a href="http://wordpress.org/extend/plugins/training-peaks-connect/">TrainingPeaks Connect Plug-in Page</a><p>
...
function admin_manage() {
            ...
            if ( empty($mode) or ($mode == 'main') ) {
                ...
                print ' <h2>';
                print _e('Manage TrainingPeaks Accounts','training-peaks-connect');
                print '</h2>';
                ...
                print '</th>';
                print '    <th scope="col" align="left">';
                print _e('<center>TrainingPeaks Profile URL</center>','training-peaks-connect');
                print '</th>';
                print '    <th scope="col" align="left">';
                ...
                if ( $alloptions['accounts'] || $newaccount ) {
					...
					foreach ($alloptions['accounts'] as $key => $val) {
                        ...
                        if ( isset($edit_id) && $edit_id == $key ) {
                            ...
                            print '<td><input size="20" maxlength="250" id="training-peaks-connect-tplink" name="training-peaks-connect-tplink" type="text" value="'.$val['tplink'].'" /></td>';
                            print '<td><input size="20" maxlength="100" id="training-peaks-connect-tplinktext" name="training-peaks-connect-tplinktext" type="text" value="'.$val['tplinktext'].'" /></td>';
                    ...
                    if ( $newaccount ) {
                        ...
                        print '<td><input size="20" maxlength="250" id="training-peaks-connect-tplink" name="training-peaks-connect-tplink" type="text" value="" /></td>';
                ...
                print ' <b>-training-peaks-connect#myname-</b> or <b>-trainingpeaks-connect#myname-</b>. ';
                ...
                print '<tr><td align="right" valign="top" width=205><b>User Name: </b></td><td valign="top">This is the name used for logging into TrainingPeaks </td></tr>';
                print '<tr><td align="right" valign="top" width=205><b>Password: </b></td><td valign="top">This is the password used for logging into TrainingPeaks.</td></tr>';
                print '<tr><td align="right" valign="top" width=205><b>TrainingPeaks Profile URL: </b></td><td valign="top">Provide a URL link to the user\'s public TrainingPeaks Profile, if available.  Example: http://www.trainingpeaks.com/JDoe</td></tr>';
                print '<tr><td align="right" valign="top" width=205><b>Text for Link to Profile: </b></td><td valign="top">Provide text for the link that visitors will click on to view a public TrainingPeaks Profile.  Example: See more workouts...</td></tr>';
                ...
                print '<tr><td align="right" valign="top" width=205><b>Publicly display workout comments: </b></td><td valign="top">Check this box if you would like to be able to see comments for each workout that have been input by the user on TrainingPeaks. </td></tr>';
    ...
    $tag1 = '/-trainingpeaks-connect(|#.*?)-/';
    $result = preg_replace_callback($tag, array(&$this, 'inline_replace_callback'), $data);
    $result = preg_replace_callback($tag1, array(&$this, 'inline_replace_callback'), $result);
      return $result;
    }
    ...
    function display_workouts($accountname)
    {
            ...
            if ( ! count($matching_accounts) ) {         
                ...
                {    
            	return "<ul>--TRAININGPEAKS CONNECT ERROR: Unknown account name--</ul>";
                }
            }
            ...
            if ($this->httpErrorCode != 0){
            	if ($settings['showerrors'] == "checked")
            	{
            	$result .= '--TRAININGPEAKS CONNECT ERROR: Cannot access TrainingPeaks--';
            	}
            	else
            	{
            	    return '';
...
$TP_instance &= new TrainingPeaks_Connect_Plugin();
}
?>